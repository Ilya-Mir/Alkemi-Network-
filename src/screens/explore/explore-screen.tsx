...
export const ExploreScreen = ({trendData, accountData}: { trendData: any, accountData?: any }) => {
    ...
    useEffect(() => {
        const postsTimeoutId = window.setTimeout(() => setData(POST_DATA), 2000)
        const tagsTimeoutId = window.setTimeout(() => setTrendTagData(TAGS), 2000)

        return () => {
            window.clearTimeout(postsTimeoutId)
            window.clearTimeout(tagsTimeoutId)
        }
    }, [])
                    ...
                    <Grid sx={{px: 2.5, marginBottom: "85px"}} item lg={8} md={8} sm={12} xs={12}>
                        {
                            data ?
                                ...
                                :
                                custom_3.map((_, index) => <LoadPost key={index}/>)
                        }
